...
- [ ] 2.7 (this file) Write `tasks.md` and confirm `openspec status --change pi-agent-panel` reports `isComplete: true`
- [ ] 2.8 Commit Step 1 (deletion) + Step 2 (this openspec change) as two logically separate commits per the user's "先删后改（分两步）" choice

## 3. Node sidecar skeleton (`packages/gui/sidecar/`)

- [ ] 3.1 Create `packages/gui/sidecar/package.json` with pinned dependencies `@earendil-works/pi-coding-agent`, `@earendil-works/pi-ai`, `typebox`, `zod`; dev deps `esbuild`, `typescript`, `@types/node`, `vitest`; scripts `build` (esbuild bundle to `dist/sidecar.js`), `dev`, `test`
- [ ] 3.2 Create `packages/gui/sidecar/tsconfig.json` targeting `node18` with strict mode and `module: ESNext`
...
- [ ] 3.4 Implement the stdin request loop: parse one JSON frame per line into `AgentRequest { kind: "user_message"|"slash_command"|"cancel"|"ui_response", sessionId, text?, requestId?, value? }`, dispatch to the active `AgentSession`/`AgentSessionRuntime`/active `wait` map; rejections on malformed frames MUST log to stderr (never stdout, which is reserved for `AgentEvent` JSONL)
- [ ] 3.5 Implement `src/agent-event-adapter.ts` exposing `mapPiEvent(sessionId, piEvent): AgentEventJsonL | null` for each `AgentSessionEvent.type`: `text_delta`→`text_delta`, `toolcall_end`→`tool_call`, the pi tool-result phase→`tool_result` (derive `status` and `result` from the pi `ToolResult.content` envelope), pi `done`→`done`, pi `error`→`error { message }`. Skip pi events that have no `AgentEvent` analogue (e.g. `thinking_start`). Unit-test every branch.
- [ ] 3.6 Implement `src/slash-commands.ts` registering `/new` (`runtime.newSession()` then emit a `session_id` announcement event), `/sessions` (call `SessionManager.list(cwd)`, emit a `ui_request { kind: "select_session", payload: { sessions } }`, await the matching `ui_response` on stdin, then `runtime.switchSession(value)`), `/compact` (call `session.compact(args)`), `/clear`, `/help`, `/model` (`session.cycleModel()` or `setModel`)
- [ ] 3.7 Implement `src/stdout-writer.ts`: one helper `emitAgentEvent(sessionId, event: AgentEvent)` that writes a single JSONL line to stdout and flushes (never batches — preserve real-time streaming); one helper `emitUiRequest(requestId, kind, payload)` for the round-trip channel
- [ ] 3.8 Implement `src/session-lifecycle.ts` wrapping `createAgentSession`, `session.subscribe(listener → adapter → stdout-writer)`, `session.prompt(text)`, `session.abort()`, `runtime.newSession()`, `runtime.switchSession(id)`
- [ ] 3.9 Verify the sidecar boots with a fake stdin frame and emits at least one synthetic `text_delta` event end-to-end via a vitest test using a stub spawner
- [ ] 3.10 Add a snapshot test that the adapter produces the exact JSONL shapes specified by `agent-pi-sidecar#agentevent-adaptation-from-pi-to-the-frontend-contract`

## 4. Tauri sidecar lifecycle (`packages/gui/sidecar/` + `packages/gui/src-tauri/`)

- [ ] 4.1 Configure `packages/gui/src-tauri/tauri.conf.json` `externalBin` to target the bundled sidecar entry per target triple (placeholder for now; real binary wired in 4.x after the bundler produces `dist/sidecar.js` + a Node runtime)
- [ ] 4.2 Create `packages/gui/src-tauri/src/sidecar.rs` exporting a `SidecarHandle` struct holding `tokio::process::Child`, `ChildStdin`, a reader task driving `ChildStdout` → `AgentEvent` → `agent:event`, and an active-session map (`Mutex<HashMap<SessionId, ActiveSession>>`)
- [ ] 4.3 In `sidecar.rs` implement `boot_sidecar(app: &AppHandle) -> Result<SidecarHandle, String>` that spawns the bundled Node sidecar via `tokio::process::Command` (no argv carries the key), hands stdin to the writer half, spawns a `tokio::spawn` reader task that pumps stdout line-by-line into the `AgentEvent` channel
- [ ] 4.4 Implement `agent_new_session(working_folder: Option<String>) -> { session_id: String Tauri command: read key from keychain, read config, resolve working_folder (arg then `config.last_working_folder`), write the first BYOK stdin frame `{ apiKey, provider, baseUrl, model, cwd, sessionId: null }`, await the sidecar's `session_id` announcement, store the active session, return the id
- [ ] 4.5 Implement `agent_send(session_id, text) -> ()` Tauri command: validate the session is active, write a `{ kind: "user_message", sessionId, text }` stdin frame, return immediately (do NOT block on the agent turn)
- [ ] 4.6 Implement `agent_cancel(session_id -> ()` Tauri command: write a `{ kind: "cancel", sessionId }` stdin frame; the next stdout event for that session is `error { message: "cancelled" }`
- [ ] 4.7 Implement `agent_list_sessions(working_folder: Option<String>) -> Vec<SessionMeta>` Tauri command: write a `{ kind: "list_sessions", cwd }` stdin frame, await the matching `list_sessions_reply` on stdout, map pi `SessionMeta` to our five-field shape ({ id, title, created_at, updated_at, message_count })
- [ ] 4.8 Implement `agent_load_session(session_id, working_folder: Option<String>) -> Vec<ChatRow>` Tauri command: write a `{ kind: "load_session", sessionId, cwd }` stdin frame, await the matching reply, return rows in submission order
- [ ] 4.9 Implement the stdout reader task: parse one JSONL line at a time into either an `AgentEvent` (forwarded via `app.emit("agent:event", { session_id, event })`) or a `ui_request` (forwarded via `app.emit("agent:ui_request", { request_id, kind, payload })`); on sidecar EOF emit `agent:event { session_id: <last active>, event: { type: "error", message: "agent process exited" } }`
- [ ] 4.10 Implement `agent_reply_ui(request_id, value)` Tauri command: write a `{ kind: "ui_response", requestId, value }` stdin frame so `/sessions` and future prompts round-trip
- [ ] 4.11 Update `packages/gui/src-tauri/src/lib.rs`: `mod sidecar;`, register the six Tauri commands (`agent_new_session`, `agent_send`, `agent_cancel`, `agent_list_sessions`, `agent_load_session`, `agent_reply_ui`), call `boot_sidecar` once in `setup` via `tauri::Builder::default().setup(...)`, store `SidecarHandle` in `tauri::State`
- [ ] 4.12 Update `packages/gui/src-tauri/Cargo.toml`: re-add `tokio = { workspace = true, features = ["process", "io-util", "rt"] }` and `futures.workspace = true` if the reader task needs them; otherwise confirm `tauri`'s transitive tokio suffices (run `cargo check`)
- [ ] 4.13 Verify `cargo check -p learn-anything-gui` is green; verify a manual `pnpm tauri dev` boots the sidecar without errors (or with a clear error if Node is missing — generate a user-actionable message)
- [ ] 4.14 Backend tests in `sidecar.rs` for: no-key error path, no-working-folder error path, sidecar-EOF surfacing `agent:event error`, ui_request round-trip returning the chosen `request_id`'s value
