...
const subtitle = computed(
  () =>
    `${overall.value.topics} topics · ${overall.value.totalConcepts} concepts · ${overall.value.mastered} mastered · overall ${overall.value.percentage}%`,
);
...
<template>
  <div class="flex h-full flex-col overflow-y-auto pr-6">
    ...
    <div class="mb-8 flex items-end justify-between">
      <div>
        <h1 class="m-0 text-2xl font-semibold tracking-tight">Your learning</h1>
      ...
      <button
        ...
        @click="newTopic"
      >
        <span class="text-base leading-none">+</span> New topic
      </button>
    </div>

    <!-- Initial load (no rows yet) -->
    <div v-if="loading && !hasRows" class="opacity-60">Loading…</div>
    ...
    <p v-else-if="!hasRows" class="max-w-md text-sm text-(--color-pencil)">
      No topics yet. Ask the agent to <span class="font-mono text-(--color-ink)">/learn-topic</span>
      something new.
    </p>
    ...
    <template v-else>
      ...
      <p class="mt-10 max-w-md text-sm text-(--color-pencil)">
        Pick a topic to open its workspace, or ask the agent to &nbsp;<span
          class="font-mono text-(--color-ink)"
          >/learn-topic</span
        >&nbsp; something new.
      </p>
    </template>
