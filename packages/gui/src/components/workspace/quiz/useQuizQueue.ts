 ...
 *
 * Adapted from the CLI: `fetchQuizDeck` is replaced by the GUI's
 * `siteQuizDeck` invoke wrapper (which takes a `workingFolder` and resolves
 * `null` for a missing deck → treated as an error phase). */
...
export function useQuizQueue(
  ...
  async function loadCurrent(): Promise<void> {
    ...
    try {
      ...
      // QueueItem.path keeps the `quizzes/` axis prefix — strip it for `rest`.
      const rest = item.path.replace(/^quizzes\//, '');
      const raw = await siteQuizDeck(topicSlug, rest, workingFolder);
      if (!raw) {
        currentDeck.value = null;
        phase.value = 'error';
        return;
      }
      currentDeck.value = raw as QuizDeck;
      phase.value = 'quiz';
    } catch {
      currentDeck.value = null;
      phase.value = 'error';
