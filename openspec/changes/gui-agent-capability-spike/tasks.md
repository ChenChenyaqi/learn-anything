...
- [ ] 1.1 Create the Cargo workspace: add a root `Cargo.toml` (workspace) covering `crates/*` and `packages/gui/src-tauri`, without disturbing the existing pnpm CLI build
- [ ] 1.2 Scaffold the `crates/learn-agent` Rust library crate (`lib.rs`, `Cargo.toml` with `serde`, `schemars`, `rig-core`, `tokio`, `anyhow`)
